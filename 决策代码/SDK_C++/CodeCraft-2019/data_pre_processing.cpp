...
void myReadparam(ifstream &infile, vector<int> &Para_A, vector<int> &Para_B, vector<int> &Para_C, vector<int> &Para_D, vector<int> &Para_E, vector<int> &Para_F, vector<int> &Para_G)
{
    ...
    while (getline(infile, s))
    {
        ...
        if (s[0] != '#')
        {
            ...
            s.erase(s.end() - 1);
            SplitString(s, v, ",");
            Para_A.push_back(atoi(v[0].c_str()));
            Para_B.push_back(atoi(v[1].c_str()));
            Para_C.push_back(atoi(v[2].c_str()));
            Para_D.push_back(atoi(v[3].c_str()));
            Para_E.push_back(atoi(v[4].c_str()));
            Para_F.push_back(atoi(v[5].c_str()));
            Para_G.push_back(atoi(v[6].c_str()));
            i++;
        }
    }
    infile.close();
