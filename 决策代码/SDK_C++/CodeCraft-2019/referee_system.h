...
{
public:
    vector_finder(const int m_i_a):m_i_a(m_i_a) {}
    bool operator()(const Car &s_car)
    {
        if(s_car.car_id == m_i_a)
            return true;
        else
            return false;
    }
...
void Loading_localanswer(vector<Road> &m_road,vector<Answer> &m_answer,vector<presetCar> &m_pcar, vector<Car> &m_car,vector<Car> &op_car, map<int, int> &car_map,map<int, int> &road_map);
